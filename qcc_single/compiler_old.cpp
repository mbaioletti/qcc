...
static Solution* find_solution(pair<int,int> *init_state, const chrono::steady_clock::time_point& deadline)
{
    auto *s = new Solution(compile_data.problem, init_state);
    int tm  = 0;
           ...
           s->num_swaps    <= compile_data.max_num_swaps)
    {
        ...
        auto values = s->evaluate_executable(ts, ts, compile_data.prune, compile_data.accurate);
...
static Solution* optimize()
{
    ...
    auto deadline = t_start + chrono::milliseconds(static_cast<int>(compile_data.timeout*1000));
    ...
    while (chrono::steady_clock::now() < deadline) {
        ...
        Solution *s = find_solution(init, deadline);
        ...
        if (s) {
            ...
            if (better) {
                ...
                cout << (compile_data.obj_fun == "swaps" ? "swaps " : "depth ")
                     ...
                     << " after " << num_sol_found << " solutions and " << elapsed << " ms" << endl;
                     //<< "  |  beta="  << best_beta
                     //<< "  gamma="   << best_gamma
