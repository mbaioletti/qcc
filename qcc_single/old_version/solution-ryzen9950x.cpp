...
vector<HActivity> Solution::evaluate_executable(int tm, int pruning) {
    ...
    int k1min=INT_MAX, k2min=INT_MAX;
    int k1max=0, k2max=0;
    ...
    for (auto e : supported) if(e->status==EXECUTABLE) {
        if (executable_at_time(e,tm)) {
            ...
            values.push_back({e,dsum,dmin,0.0});
            /*if(e->gate->arity==2) {
    ...
    for (auto sw : possible_swaps) {
        ...
        if (executable_at_time(sw,tm)) {
            ...
            k2min = min(k2min, dmin);
            k2max = max(k2max, dmin);
            values.push_back({sw,dsum,dmin,0.0});
        }
    }
    double diff1=k1max-k1min;
    double diff2=k2max-k2min;
    for (auto &v : values) {
        double r1 = diff1==0.0 ? 1.0 : (v.dsum-k1min)/diff1;
        double r2 = diff2==0.0 ? 1.0 : (v.dmin-k2min)/diff2;
        v.heur=10*r1+r2;
    }
